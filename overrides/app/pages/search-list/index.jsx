...
} from '../../components/amplience/product-tile'
...
const SearchList = (props) => {
    ...
    const categories = ampContext.client.categories
    ...
    const [ampPages, setAmpPages] = useState([])
    ...
    /**************** Amplience Query Actions ****************/
    const getAmpPages = ({queryKey}) => {
        const [searchpages, locale, query] = queryKey
        return ampContext.client.getSearchableContentPages(locale, query).then((res) => res).then((res) => res)
    }

    const ampPageSearchQuery = useQuery(
        ['searchpages', locale.id, searchQuery],
        getAmpPages,
        {
            enabled: !!searchQuery
        }
    ...
    useEffect(() => {
        //console.log('pages', ampPageSearchQuery)
        const sorted = ampPageSearchQuery.data?.sort((a,b) => 
            (a.content?.seo?.title ?? '').localeCompare(b.content?.seo?.title ?? '')
        )
        console.log('sorted', sorted)
        setAmpPages(sorted)
    }, [ampPageSearchQuery.data])

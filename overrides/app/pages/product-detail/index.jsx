...
import {useAmpRtv} from 'amplience-api-react/dist/utils/rtv'
...
const ProductDetail = () => {
    ...
    /*************************** Amplience Content ********************/
    const [productPdp, setProductPdp] = useState()

    const getPdpContent = ({queryKey}) => {
        const [pdpcontent, locale, id] = queryKey

        return ampContext.client.fetchContent(
            [{key: `pdp/content/${id?.toUpperCase()}`}],
            {locale: locale}
        ).then((res) => res).then((res) => res)
    }

    const pdpContent = useQuery(
        ['pdpcontent', locale.id, productId],
        getPdpContent,
        {
            enabled: !!productId
        }
    ...
    useAmpRtv(
        ...
        undefined,
        [productPdp, ampContext.groups]
    )

    useEffect(() => {
        const content = pdpContent.data?.pop()
        if(content?.active){
            setProductPdp(content)
        }
    }, [pdpContent.data])

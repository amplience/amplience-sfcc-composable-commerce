...
const formatSuggestions = (searchSuggestions, pageSuggestions, input) => {
    return {
        ...
        pageSuggestions: pageSuggestions?.map((page) => {
...
const Search = (props) => {
    ...
    const pageSuggestion = useQuery(
        ['pagesuggestions', searchQuery],
        () => {
            return ampContext.client.getSearchableContentPages(locale, searchQuery)
    ...
    const searchSuggestions = useMemo(
        () => formatSuggestions(searchSuggestion.data, pageSuggestion.data, searchInputRef?.current?.value),
        [searchSuggestion, pageSuggestion]
    )

    // check if popover should open if we have suggestions
    useEffect(() => {
        shouldOpenPopover()
    }, [searchQuery, searchSuggestion.data, pageSuggestion.data])
